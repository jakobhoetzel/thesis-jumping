...
if weight_path == "":
    ...
    command_set = []
    with open('data/full/randomCommandData.csv') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            command_set.append(row)

    for step in range(max_steps):
        frame_start = time.time()

        command = np.array(command_set[step], dtype=np.float32)
        ...
        writer.writerow([command[0], command[1], command[2]])
