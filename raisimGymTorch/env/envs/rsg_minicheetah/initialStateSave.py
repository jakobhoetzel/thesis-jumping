...
if False:  # weight_path == "":
    ...
    numRuns = 500
    for numberRuns in range(numRuns):
        env.reset()
        for step in range(max_steps):
            ...
            #     env.set_command(command, testNumber=1)
            if step == 0:
                command_Vx = np.random.uniform(3.0, 3.5, 1)
                command_Vy = np.random.uniform(-0.05, 0.05, 1)
                command_yaw = np.random.uniform(-0.05, 0.05, 1)
                command = np.array([command_Vx, command_Vy, command_yaw], dtype=np.float32)
                env.set_command(command, testNumber=1)
            ...
            # time.sleep(0.002) #0.05 ONLY TO SEE, NOT FOR REAL SPEED!
            # if step > 550:
            #     time.sleep(wait_time)
        # time.sleep(0.1)
        print(numberRuns)
